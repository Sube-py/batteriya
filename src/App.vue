...
watch(
  ...
  () => {
    ...
    total.value = simulateCharging(form.batteryNum, form.maxPower, form.time)
  },
  { immediate: true }
)
...
watch(
  () => [ui.minutes, form],
  debounce(() => {
    console.log(ui.minutes)
    if (!form.batteryNum || !form.maxPower || !form.time) return
    simulateCharging(form.batteryNum, form.maxPower, form.time, ui)
  }, 100),
  ...
  <div class="h-screen w-full bg-gray-100 flex">
    ...
    <div class="flex-1 flex flex-col p-4 h-full w-full overflow-scroll">
      ...
      <div class="flex flex-col flex-1">
        <div>总功率: {{ ui.power }}W</div>
        <div>已充: {{ ui.charged }}块</div>
      </div>
    </div>
  </div>
</template>
